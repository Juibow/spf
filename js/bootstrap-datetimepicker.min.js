! function(e) {
    ...
    g.prototype = { constructor: g, _events: [], _attachEvents: function() { this._detachEvents();
            ...
            this.update();
            this.updateNavArrows() }, setDaysOfWeekDisabled: function(i) { this.daysOfWeekDisabled = i || [];
            ...
            if (!this.zIndex) {
                ...
                this.zIndex = i + 10 }
            var m, l, k, j;
            if (this.container instanceof e) { j = this.container.offset() } else { j = e(this.container).offset() }
            if (this.component) { m = this.component.offset();
                k = m.left;
                if (this.pickerPosition == "bottom-left" || this.pickerPosition == "top-left") { k += this.component.outerWidth() - this.picker.outerWidth() } } else { m = this.element.offset();
                k = m.left }
            if (k + 220 > document.body.clientWidth) { k = document.body.clientWidth - 220 }
            if (this.pickerPosition == "top-left" || this.pickerPosition == "top-right") { l = m.top - this.picker.outerHeight() } else { l = m.top + this.height }
            l = l - j.top;
            k = k - j.left;
            if (!b(this.element)) { l = l + document.body.scrollTop }
            this.picker.css({ top: l, left: k, zIndex: this.zIndex }) }, update: function() {
            var i, j = false;
            ...
            if (this.formatViewType == "time") {
                ...
                this.picker.find(".datetimepicker-minutes thead th:eq(1)").text(k) } else { this.picker.find(".datetimepicker-hours thead th:eq(1)").text(n + " " + f[this.language].months[I] + " " + u);
            ...
            while (K.valueOf() < j) {
                ...
                if ((K.valueOf() + 86400000) <= this.startDate || K.valueOf() > this.endDate || e.inArray(K.getUTCDay(), this.daysOfWeekDisabled) !== -1) { v += " disabled" }
            ...
            if (o) {
                ...
                if (k) { k.change() }
                this.element.trigger({ type: "changeDate", date: this.date }) } }, showMode: function(i) {
            if (i) {
    ...
    var d = { modes: [{ clsName: "minutes", navFnc: "Hours", navStep: 1 }, { clsName: "hours", navFnc: "Date", navStep: 1 }, { clsName: "days", navFnc: "Month", navStep: 1 }, { clsName: "months", navFnc: "FullYear", navStep: 1 }, { clsName: "years", navFnc: "FullYear", navStep: 10 }], isLeapYear: function(i) {
            ...
            if (i == "standard") {
                return /hh?|HH?|p|P|ii?|ss?|dd?|DD?|mm?|MM?|yy(?:yy)?/g } else {
                if (i == "php") {
